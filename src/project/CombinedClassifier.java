...
public class CombinedClassifier {
    ...
    public void run(){
        try{
            ...
            Instances train2= DataSource.read(train_path);
            Instances test2= DataSource.read(test_path);
            train2.setClassIndex(train1.numAttributes()-1);
            test2.setClassIndex(test1.numAttributes()-1);
            ...
            Remove rt=new Remove();
            rt.setAttributeIndices("1");

            RandomForest RF=new RandomForest();
            //RF.setUnpruned(true);

            FilteredClassifier ft = new FilteredClassifier();
            ft.setFilter(rm);
            ft.setClassifier(RF);
            ft.buildClassifier(train2);
            ...
            for(int i=0;i<test.numInstances();i++){
                ...
                double predRF = ft.classifyInstance(test2.instance(i));
                String a="anomaly";
                String actual;
                String predicted;
                String actual1;
                ...
                actual2=test2.classAttribute().value((int) test2.instance(i).classValue());
                predicted2= test2.classAttribute().value((int) predRF);
                
                if(predicted1.equalsIgnoreCase(a)||predicted.equalsIgnoreCase(a)||predicted2.equalsIgnoreCase(a))
                    predicted=a;
